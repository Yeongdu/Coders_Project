...
<body>
		...
		<div id = "main_top"> <%-- 정렬 / 언어 선택 창 --%>
			...
			<ul>
				...
				<li class = "qna_sort">
					...
					<button type="button" class="btn btn-outline-primary" id = "comment_btn" onclick = "location.href ='<%=request.getContextPath()%>/qna_list_comment.do'" disabled>답변순</button>
				</li>
			</ul>
		</div> <%-- main_top의 end --%>
	...
	<%-- BootStrap을 이용한 페이징 처리 영역 --%>
		<nav>
          <ul class="pagination">
            <li class="page-item">
              <a class="page-link" href="qna_list_comment.do?page=1" id = "page1">First</a></li>
            <c:choose>
                <c:when test="${ (page - 1) == 0}">
                    <li><a class="page-link" href="qna_list_comment.do?page=1" id = "page2">Previous</a></li>
                </c:when>
                <c:otherwise>
                    <li><a class="page-link" href="qna_list_comment.do?page=${ page - 1 }" id = "page3">Previous</a></li>
                </c:otherwise>
            </c:choose>
            <c:forEach begin="${ startBlock }" end="${ endBlock }" var="i">
                <c:if test="${ i==page }">
                    <li class="page-item active" aria-current="page">
                    <a class="page-link" href="qna_list_comment.do?page=${i }" id = "page4">${i }</a></li>
                </c:if>
                <c:if test="${ i!=page }">
                    <li class="page-item">
                    <a class="page-link" href="qna_list_comment.do?page=${i }" id = "page5">${i }</a></li>
                </c:if>
            </c:forEach>
           <c:if test="${ page < allPage }">
                <li class="page-item">
                <a class="page-link" href="qna_list_comment.do?page=${ page + 1 }" id = "page6">Next</a>
                </li>
                <li class="page-item">
                <a class="page-link" href="qna_list_comment.do?page=${ allPage }" id = "page7">End</a>
                </li>
            </c:if>
          </ul>
        </nav>
