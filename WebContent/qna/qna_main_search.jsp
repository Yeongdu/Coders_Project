...
<body id  = "qna">
		...
		<div id = "search_top" style = "display:flex; width: 750px;"> <%-- 정렬 / 언어 선택 창 --%>
					...
					<div class="qnaSearchWrite" style="margin: auto;">
							...
							<span class="input-group list-search-form w-70"> 
									<input type="text" id ="search_keyword" name = "search_keyword" class="form-control" style = "margin-top:0; width:20em;" value = "${searchkeyword }"/>
	...
	<%-- BootStrap을 이용한 페이징 처리 영역 --%>
		<nav>
          <ul class="pagination">
            <li class="page-item">
              <a class="page-link" href="qna_search.do?page=1&search_keyword=${searchkeyword }" id = "page1">First</a></li>
            <c:choose>
                <c:when test="${ (page - 1) == 0}">
                    <li><a class="page-link" href="qna_search.do?page=1&search_keyword=${searchkeyword }" id = "page2">Previous</a></li>
                </c:when>
                <c:otherwise>
                    <li><a class="page-link" href="qna_search.do?page=${ page - 1 }&search_keyword=${searchkeyword }" id = "page3">Previous</a></li>
                </c:otherwise>
            </c:choose>
            <c:forEach begin="${ startBlock }" end="${ endBlock }" var="i">
                <c:if test="${ i==page }">
                    <li class="page-item active" aria-current="page">
                    <a class="page-link" href="qna_search.do?page=${i }&search_keyword=${searchkeyword }" id = "page4">${i }</a></li>
                </c:if>
                <c:if test="${ i!=page }">
                    <li class="page-item">
                    <a class="page-link" href="qna_search.do?page=${i }&search_keyword=${searchkeyword }" id = "page5">${i }</a></li>
                </c:if>
            </c:forEach>
           <c:if test="${ page < allPage }">
                <li class="page-item">
                <a class="page-link" href="qna_search.do?page=${ page + 1 }&search_keyword=${searchkeyword }" id = "page6">Next</a>
                </li>
                <li class="page-item">
                <a class="page-link" href="qna_search.do?page=${ allPage }&search_keyword=${searchkeyword }" id = "page7">End</a>
                </li>
            </c:if>
          </ul>
        </nav>
