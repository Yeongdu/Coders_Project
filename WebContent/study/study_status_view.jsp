	...
	<div align="center">
		...
		<div style="width: 750px; display: flex; margin-left: 5px; margin-right: 5px;">
			...
			<div class="studySearchWrite" style="margin: auto;">
				<form name="search_form" method="post" action="<%=request.getContextPath()%>/study_status_search.do">
...
<!-- 							</select> -->
			...
			<button class="btn btn-outline-primary"
				onclick="location.href='<%=request.getContextPath()%>/studyBoard_list.do'">
				<i class="fa fa-search"></i> 전체 글
			</button>
		</div>
		<%-- 글쓰기 /검색 / 정렬 기능 창 end --%>
		...
		<nav>
			<ul class="pagination">
				<li class="page-item"><a class="page-link"
					href="study_status_view.do?page=1&search_field=${field}&search_keyword=${keyword }">First</a></li>
				<c:choose>
					<c:when test="${ (page - 1) == 0}">
						<li><a class="page-link" href="study_status_view.do?page=1&search_field=${field}&search_keyword=${keyword }">Previous</a></li>
					</c:when>
					<c:otherwise>
						<li><a class="page-link"
							href="study_status_view.do?page=${ page - 1 }&search_field=${field}&search_keyword=${keyword }">Previous</a></li>
					</c:otherwise>
				</c:choose>
				<c:forEach begin="${ startBlock }" end="${ endBlock }" var="i">
					<c:if test="${ i==page }">
						<li class="page-item active" aria-current="page"><a
							class="page-link" href="study_status_view.do?page=${i }&search_field=${field}&search_keyword=${keyword }">${i }</a></li>
					</c:if>
					<c:if test="${ i!=page }">
						<li class="page-item"><a class="page-link"
							href="study_status_view.do?page=${i }&search_field=${field}&search_keyword=${keyword }">${i }</a></li>
					</c:if>
				</c:forEach>
				<c:if test="${ page < allPage }">
					<li class="page-item"><a class="page-link"
						href="study_status_view.do?page=${ page + 1 }&search_field=${field}&search_keyword=${keyword }">Next</a></li>
					<li class="page-item"><a class="page-link"
						href="study_status_view.do?page=${ allPage }&search_field=${field}&search_keyword=${keyword }">End</a></li>
				</c:if>
			</ul>
		</nav>
